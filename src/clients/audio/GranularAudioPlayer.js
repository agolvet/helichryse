class GranularAudioPlayer {
  ...
  start() {
    ...
    this.position = Math.random() * this._buffer.duration;
  ...
  render(currentTime) {
    ...
    const randDuration = this.duration + (Math.random() * 2 - 1) * this.durationVar;
    const duration = Math.min(randDuration, this.buffer.duration - this.position);
    const detuneValue = this.detune + (Math.random() * 2 - 1) * this.detuneVar;
    const period = this.period + (Math.random() * 2 - 1) * this.periodVar;
    ...
    this.position = (this.position + period) % (this.buffer.duration - duration);
